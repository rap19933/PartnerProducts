...
if (!empty($arResult["DISPLAY_PROPERTIES"]["ATT_PARTNER"]["VALUE"]))
{
	...
	while($ar_props = $db_props->Fetch())
	{
		if (!empty($ar_props['VALUE_ENUM']))
		{
			if ($i == 0)
			{
				$arResult["DISPLAY_PROPERTIES"]["ATT_PARTNER"]["PROPERTY_NAME_ENUM"] = $ar_props['NAME'];
			}
			$arResult["DISPLAY_PROPERTIES"]["ATT_PARTNER"]["PROPERTY_VALUE_ENUM"][$i] = $ar_props['VALUE_ENUM'];
			$i++;
		}
		else
		{
			if ($ar_props['CODE'] == "DESCRIPTION")
			{
				$arResult["DISPLAY_PROPERTIES"]["ATT_PARTNER"]["PROPERTY_NAME"]=$ar_props['NAME'];
				$arResult["DISPLAY_PROPERTIES"]["ATT_PARTNER"]["PROPERTY_VALUE"]=$ar_props['VALUE'];
			}
		}
	}
}
