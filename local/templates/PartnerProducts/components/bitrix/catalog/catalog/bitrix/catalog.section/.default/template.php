...
//пагинация
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		?><div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>"><div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
			...
			if ($USER->IsAuthorized()){
				if (empty($arItem['PRODUCT_PROPERTIES']['ATT_PARTNER']['SET'])){
					?>
					<div class="bx_c">
						<a id="<? echo $arItem['ID']; ?>"
						   href="<?=SITE_DIR?>addoperator.php?idProduct=<?=$arItem['ID'];?>&idSection=<?=$arItem['IBLOCK_SECTION_ID'];?>"
						   class="bx_bt_button bx_medium" >Стать оператором товара</a>
					</div>
					<?
				}
	...
	{
		?><? echo $arResult["NAV_STRING"]; ?></article><?
	}
}
